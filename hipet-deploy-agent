...
# HiPet Agent Service Deployment Script
# 用于在 EC2 上部署 Agent Service
...
SERVICE_DIR="$PROJECT_DIR/agent-service"
TMUX_SESSION="hipet"
TMUX_WINDOW="agent"
SERVICE_PORT=8001
...
check_project_dir() {
    ...
    if [ ! -d "$SERVICE_DIR" ]; then
        print_error "Agent service 目录不存在: $SERVICE_DIR"
        exit 1
    fi
...
install_dependencies() {
    ...
    if [ ! -f ".env" ]; then
        ...
        print_error "请先创建 .env 文件并配置必要参数"
        print_info "必需的环境变量："
    ...
    # 检查 Google 认证文件
    if [ -f ".env" ]; then
        GOOGLE_CRED_PATH=$(grep "^GOOGLE_APPLICATION_CREDENTIALS=" .env | cut -d '=' -f2 | tr -d '"' || true)
        if [ ! -z "$GOOGLE_CRED_PATH" ] && [ ! -f "$GOOGLE_CRED_PATH" ]; then
            print_warning "Google 认证文件不存在: $GOOGLE_CRED_PATH"
            print_info "请确保认证文件路径正确"
        fi
    fi
    ...
    # 验证关键依赖
    print_info "验证依赖安装..."
    python3 -c "import fastapi, vertexai, loguru" 2>/dev/null || {
        print_error "依赖验证失败，请检查 requirements.txt"
        exit 1
    }
...
start_service() {
    ...
    # 加载环境变量并启动（使用 set -a 自动导出所有变量）
    if [ -f ".env" ]; then
        tmux send-keys -t "${TMUX_SESSION}:${TMUX_WINDOW}" "set -a && source .env && set +a" C-m
    fi
    tmux send-keys -t "${TMUX_SESSION}:${TMUX_WINDOW}" "python3 main.py" C-m
    ...
    print_info "API 文档: http://localhost:$SERVICE_PORT/docs"
}

# 主函数
main() {
    echo "=========================================="
    echo "  HiPet Agent Service 部署脚本"
    echo "=========================================="
    echo ""
