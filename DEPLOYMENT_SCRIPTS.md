# 部署脚本使用说明

## 📋 概述

项目提供了两个自动化部署脚本，用于在 EC2 上快速部署服务：

- `deploy_backend.sh` - Backend Service 部署脚本
- `deploy_agent.sh` - Agent Service 部署脚本

## 🎭 Tmux 组织结构说明

**当前设计**：两个服务使用**同一个 tmux session (`hipet`)**，但使用**不同的 window** 区分：
- `hipet:backend` - Backend Service (端口 8000)
- `hipet:agent` - Agent Service (端口 8001)

**优点**：
- ✅ 两个服务在一个 session 中，方便统一管理
- ✅ 可以用 `tmux attach -t hipet` 一次性查看所有服务
- ✅ 切换窗口快速（Ctrl+B, n/p 或数字键）
- ✅ 便于同时监控两个服务的日志

**查看和切换**：
```bash
# 连接到 session（可以看到所有服务）
tmux attach -t hipet

# 查看所有 window
tmux list-windows -t hipet
# 输出示例：
# 0: backend*   (1 panes) [80x23]
# 1: agent      (1 panes) [80x23]

# 快速切换
# Ctrl+B, 0 - backend
# Ctrl+B, 1 - agent
# Ctrl+B, w - 选择窗口列表
```

**如果需要独立的 session**（修改脚本中的 `TMUX_SESSION` 变量）：
- Backend: `TMUX_SESSION="hipet-backend"`
- Agent: `TMUX_SESSION="hipet-agent"`
- 分别连接：`tmux attach -t hipet-backend` 和 `tmux attach -t hipet-agent`

## 🚀 功能特性

每个脚本都会自动执行以下步骤：

1. ✅ **检查项目目录** - 验证项目路径是否存在
2. ✅ **设置 Tmux 环境** - 创建或连接到 tmux session
3. ✅ **拉取最新代码** - 从 Git 仓库拉取最新代码（自动 stash 本地更改）
4. ✅ **释放端口** - 检查并停止占用端口的旧进程
5. ✅ **安装/更新依赖** - 安装所需依赖包
6. ✅ **启动服务** - 在 tmux 中启动服务

## 📦 脚本配置

### Backend Service (`deploy_backend.sh`)
- **端口**: 8000
- **Tmux Session**: `hipet`
- **Tmux Window**: `backend`
- **工作目录**: `~/hipet-backend/backend-service`

### Agent Service (`deploy_agent.sh`)
- **端口**: 8001
- **Tmux Session**: `hipet`
- **Tmux Window**: `agent`
- **工作目录**: `~/hipet-backend/agent-service`

## 🔧 使用方法

### 首次部署

1. **确保项目已克隆**:
   ```bash
   cd ~
   git clone git@github.com-hipet-backend:kingsleyli920/hipet-backend.git
   ```

2. **配置环境变量**:
   ```bash
   # Backend Service
   cd ~/hipet-backend/backend-service
   cp env.example .env
   vim .env  # 编辑配置
   
   # Agent Service
   cd ~/hipet-backend/agent-service
   vim .env  # 创建并编辑配置
   ```

3. **运行部署脚本**:
   ```bash
   # 部署 Backend Service
   cd ~/hipet-backend
   ./deploy_backend.sh
   
   # 部署 Agent Service
   ./deploy_agent.sh
   ```

### 更新部署

每次代码更新后，只需重新运行对应的部署脚本即可：

```bash
cd ~/hipet-backend
./deploy_backend.sh
./deploy_agent.sh
```

脚本会自动：
- 拉取最新代码
- 停止旧服务
- 更新依赖（如果有变化）
- 启动新服务

## 👀 查看服务日志

```bash
# 连接到 tmux session（包含 backend 和 agent 两个窗口）
tmux attach -t hipet

# 查看所有 window 列表（按 Ctrl+B, w）:
# 会显示：
# 0: backend*   - Backend Service (当前激活)
# 1: agent      - Agent Service

# 在 tmux 中切换 window:
# Ctrl+B, 0 - 切换到 window 0 (backend)
# Ctrl+B, 1 - 切换到 window 1 (agent)
# Ctrl+B, n - 切换到下一个 window
# Ctrl+B, p - 切换到上一个 window
# Ctrl+B, w - 显示窗口列表，用方向键选择

# 或者直接切换到指定 window（不进入 tmux）:
tmux select-window -t hipet:backend  # 切换到 backend
tmux select-window -t hipet:agent    # 切换到 agent

# 查看当前 session 的所有 window（不进入 tmux）:
tmux list-windows -t hipet

# 分离 session（服务继续运行）:
# Ctrl+B, d
```

## 🔍 验证服务运行

```bash
# 检查端口占用
sudo ss -tlnp | grep -E '8000|8001'

# 测试健康检查
curl http://localhost:8000/health/
curl http://localhost:8001/health/
```

## ⚠️ 注意事项

1. **环境变量**: 
   - Backend Service 需要有有效的 `.env` 文件
   - Agent Service 需要有 `.env` 文件，并且 `GOOGLE_APPLICATION_CREDENTIALS` 指向的文件必须存在

2. **端口冲突**: 
   - 脚本会自动停止占用端口的进程
   - 如果服务启动失败，检查端口是否被其他进程占用

3. **依赖安装**:
   - 首次运行会安装所有依赖，可能需要一些时间
   - 后续运行只会安装新增的依赖

4. **代码更改**:
   - 脚本会自动 stash 未提交的本地更改
   - 如果有重要的本地更改，请在运行脚本前手动提交或备份

## 🐛 故障排查

### 问题 1: 脚本执行权限不足

```bash
chmod +x deploy_backend.sh deploy_agent.sh
```

### 问题 2: Git 拉取失败

检查 Deploy Key 是否正确配置：
```bash
ssh -T git@github.com-hipet-backend
```

### 问题 3: 依赖安装失败

手动检查并安装：
```bash
# Backend
cd ~/hipet-backend/backend-service
bun install

# Agent
cd ~/hipet-backend/agent-service
pip3 install -r requirements.txt
```

### 问题 4: 服务启动失败

查看 tmux 日志：
```bash
tmux attach -t hipet
# 切换到对应的 window 查看错误信息
```

### 问题 5: 端口仍然被占用

手动停止进程：
```bash
# 查找进程
sudo lsof -i :8000  # Backend
sudo lsof -i :8001  # Agent

# 停止进程
sudo kill -9 <PID>
```

## 📝 脚本执行顺序说明

脚本的执行顺序经过优化，确保部署流程的可靠性：

1. **检查项目目录** - 首先验证环境是否正确
2. **设置 Tmux** - 准备运行环境
3. **拉取代码** - 更新代码（在安装依赖之前）
4. **释放端口** - 停止旧服务（在安装依赖之前，避免端口冲突）
5. **安装依赖** - 确保依赖是最新的
6. **启动服务** - 最后启动服务

这个顺序确保了：
- 先更新代码再安装依赖（依赖可能与代码版本相关）
- 先停止旧服务再安装依赖（避免资源冲突）
- 依赖安装完成后才启动服务（确保环境完整）
